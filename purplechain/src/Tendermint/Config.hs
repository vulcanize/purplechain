...
module Tendermint.Config where
...
storeConfigFile :: MonadIO m => String -> Config -> m ()
storeConfigFile configPath = liftIO . T.writeFile configPath . Toml.encode configCodec

loadConfigFile :: MonadIO m => String -> m Config
loadConfigFile configPath = liftIO $ do
  toml <- T.readFile configPath
...
_URI2Text = BiMap
    ...
    , backward = left (ArbitraryError . T.pack . show) . mkURI
    }
  where
